...
function cards() {
  ...
  return `<div class="content-wrap wide"><section class="cards-header"><p class="eyebrow">CARD CENTER</p><h1>抽卡中心</h1><p>先和組員討論：<strong>下一張想抽什麼？</strong> 不需要按照「人→事→時→地→物」的順序。</p></section><section class="main-draw-panel"><h2>強制關聯｜人・事・時・地・物</h2><p class="hint">選一個類別後，再單獨抽取。看到結果後先討論，再決定下一個類別。</p><div class="category-grid">${mainKeys.map(key => `<button class="category-tile ${draws[key]?'has-value':''} ${locks[key]?'locked':''}" data-active="${key}"><span>${deckLabels[key]}</span><strong>${esc(draws[key] || '尚未抽取')}</strong><small>${locks[key]?'已鎖定':'點選這一類'}</small></button>`).join('')}</div>${activeDeck && mainKeys.includes(activeDeck) ? drawBox(activeDeck) : ''}<div class="current-set"><div class="current-head"><h3>我們現在有什麼？</h3><button class="text-button" id="resetMain">清除本組</button></div>${drawn.length?`<div class="chips">${drawn.map(key => `<span><b>${deckLabels[key]}</b> ${esc(draws[key])} ${locks[key]?'🔒':''}</span>`).join('')}</div>`:'<p>還沒有抽卡。先討論想從哪一類開始。</p>'}</div></section><section class="extra-section"><h2>其他牌組</h2><div class="extra-grid">${extraKeys.map(key => `<div class="extra-card"><h3>${deckLabels[key]}</h3><p>${esc(draws[key] || '尚未抽取')}</p><button class="secondary" data-draw="${key}">抽一張</button></div>`).join('')}</div></section><section class="rule-note"><strong>提醒</strong><p>強制關聯的重點不是抽到容易的牌，而是嘗試連結原本看起來不相干的元素。是否允許重抽、可以重抽幾次，由老師在活動開始前決定。</p></section></div>`
}
function toolkit() { const tools=[['三句故事','第一句發生一件事 → 第二句事情改變 → 第三句出現意外'],['強制關聯','每加入一個元素，都問：「它改變了什麼？」'],['關聯檢查','拿掉這個元素，故事是否仍差不多？如果是，就繼續追問為什麼它一定要存在。'],['因為 → 所以 → 但是','「所以」建立因果；「但是」加入阻礙。'],['人物三問','他想要什麼？害怕什麼？不能讓別人知道什麼？最後，他做了什麼選擇？'],['同儕回饋','我注意到……／我很好奇……／我想知道……']]; return `<div class="content-wrap"><section class="cards-header"><p class="eyebrow">TOOLKIT</p><h1>活動工具箱</h1><p>忘記規則時，不必翻回整個單元。</p></section><div class="tool-list">${tools.map(([t,d])=>`<article><h2>${t}</h2><p>${d}</p></article>`).join('')}</div></div>` }
