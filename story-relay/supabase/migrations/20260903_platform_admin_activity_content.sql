-- Story Relay: platform administrator read-only inspection of complete activity content.
-- This intentionally does not broaden table RLS. Platform admins read through one audited RPC.
      ...
      select jsonb_agg(
        jsonb_build_object(
          ...
          'members', coalesce((
            ...
            left join public.profiles mp on mp.id = gm.user_id
            left join public.platform_member_identities mpi on mpi.user_id = gm.user_id
              ...
              jsonb_build_object(
                ...
                'segments', coalesce((
                  select jsonb_agg(
                    ...
                    ) order by sg.sequence_no, sg.submitted_at, sg.id
                  )
                ...
                'rounds', coalesce((
                  select jsonb_agg(
                    ...
                    ) order by rr.round_no, rr.started_at, rr.id
                  )
