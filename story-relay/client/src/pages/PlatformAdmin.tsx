...
import { Activity, Download, Loader2, RefreshCw, RotateCcw, Search, ShieldCheck, Trash2, UserPlus, Users } from "lucide-react";
        ...
        {!authorized ? <section className="mt-10 rounded-3xl border border-[#D8D2C6] bg-[#FFFDF8] p-8"><ShieldCheck className="text-[#A64E3C]"/><h1 className="mt-5 font-serif text-3xl font-semibold">平台管理者權限</h1><p className="mt-3 text-sm text-[#68746B]">目前登入帳號沒有平台管理者權限。</p></section> : <>
          ...
          {tab==="activities" && <section className="mt-6 rounded-3xl border border-[#D8D2C6] bg-[#FFFDF8] shadow-sm">
            ...
            <div className="overflow-x-auto"><table className="min-w-[1220px] w-full text-left text-sm"><thead className="bg-[#F3EEE5] text-[11px] text-[#777F77]"><tr><th className="px-5 py-3">活動</th><th className="px-4 py-3">主持人</th><th className="px-4 py-3">狀態</th><th className="px-4 py-3">參與者</th><th className="px-4 py-3">小組</th><th className="px-4 py-3">段落</th><th className="px-4 py-3">時間</th><th className="px-4 py-3 text-right">操作</th></tr></thead><tbody>
              {!showDeleted && (filteredActivities as ActivityStat[]).map(row => <tr key={row.activity_id} className="border-t border-[#EEE8DE]"><td className="px-5 py-4"><div className="font-semibold">{row.name||"未命名活動"}</div><div className="font-mono text-xs text-[#A06B59]">{row.code}</div></td><td className="px-4 py-4"><div>{row.host_name||"—"}</div><div className="font-mono text-[10px] text-[#7B827B]">{row.host_email||"尚未同步"}</div></td><td className="px-4 py-4">{row.status}</td><td className="px-4 py-4">{row.participant_count}</td><td className="px-4 py-4">{row.group_count}</td><td className="px-4 py-4">{row.segment_count}</td><td className="px-4 py-4 text-xs text-[#68746B]">{formatDate(row.last_activity_at)}</td><td className="px-4 py-4 text-right"><button disabled={busyId===row.activity_id} onClick={()=>void deleteActivity(row)} className="inline-flex items-center gap-1.5 rounded-lg border border-[#D7A89D] px-3 py-2 text-xs font-semibold text-[#8D4033] hover:bg-[#F7E5DF] disabled:opacity-50"><Trash2 size={13}/>刪除活動</button></td></tr>)}
              {showDeleted && (filteredActivities as DeletedActivity[]).map(row => <tr key={row.activity_id} className="border-t border-[#EEE8DE]"><td className="px-5 py-4"><div className="font-semibold">{row.name||"未命名活動"}</div><div className="font-mono text-xs text-[#A06B59]">{row.code}</div></td><td className="px-4 py-4"><div>{row.host_name||"—"}</div><div className="font-mono text-[10px] text-[#7B827B]">{row.host_email||"尚未同步"}</div></td><td className="px-4 py-4">原狀態：{row.previous_status||"closed"}</td><td className="px-4 py-4">{row.participant_count}</td><td className="px-4 py-4">{row.group_count}</td><td className="px-4 py-4">{row.segment_count}</td><td className="px-4 py-4 text-xs text-[#68746B]">刪除：{formatDate(row.deleted_at)}<br/>剩 {daysLeft(row.purge_after)} 天</td><td className="px-4 py-4 text-right"><button disabled={busyId===row.activity_id} onClick={()=>void restoreActivity(row)} className="inline-flex items-center gap-1.5 rounded-lg bg-[#355447] px-3 py-2 text-xs font-semibold text-white disabled:opacity-50"><RotateCcw size={13}/>恢復</button></td></tr>)}
